...
inline uint32_t rotate_left(uint32_t value, int shift) {return value << shift | value >> (32-shift);}
...
std::string get_md5(const std::string& in)
{
	...
	int64_t bit_length = (uint32_t)(length)*8;
	...
	uint32_t *X=(uint32_t*)(msg_for_decode);
